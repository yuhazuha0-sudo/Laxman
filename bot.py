...
FILES_JSON = "files.json"
MAX_IMAGES = 25  # per-session image limit
MAX_FILE_SIZE_BYTES = 10 * 1024 * 1024  # 10 MB
MAX_DIMENSION_PX = 4000  # maximum image dimension (px)
DEFAULT_PAGE_SIZE = "AUTO"  # AUTO | A4 | LETTER
DEFAULT_MARGIN_MM = 0

# ---------- persistence helpers ----------
def load_index():
    """Load stored PDF metadata index from JSON file."""
    try:
        with open(FILES_JSON, "r") as f:
            return json.load(f)
    except Exception:
...
def save_index_sync(index):
    """Synchronous helper to save index to JSON."""
    with open(FILES_JSON, "w") as f:
        json.dump(index, f)

async def save_index(index):
    """Async wrapper to save index without blocking."""
    await asyncio.to_thread(save_index_sync, index)

def make_slug(title: str) -> str:
    """Generate a unique slug for a PDF based on its title."""
    s = (title or "file").lower().strip().replace(" ", "_")
    return s + "_" + hashlib.md5((s + str(time.time())).encode()).hexdigest()[:6]

async def index_pdf(context, title: str, file_id: str, uploader_id: int):
    """Store sent PDF's info (file_id and metadata) in bot_data index."""
    index = context.bot_data.setdefault("files", {})
    slug = make_slug(title)
    index[slug] = {
        "file_id": file_id,
...
def ensure_image_dimensions(path: str):
    """
    If an image is larger than MAX_DIMENSION_PX, resize it proportionally.
    """
    try:
        with Image.open(path) as im:
            w, h = im.size
...
# ---------- bot handlers ----------
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Initialize per-user PDF settings if not already set
    context.user_data.setdefault("pdf_options", {
        "page_size": DEFAULT_PAGE_SIZE,
        "margin_mm": DEFAULT_MARGIN_MM
    })
    # If resuming after sending photos, preserve options but clear photos
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if file_size and file_size > MAX_FILE_SIZE_BYTES:
        await update.message.reply_text(
            f"Image bahut badi hai ({readable_size(file_size)}). "
            f"Max allowed {readable_size(MAX_FILE_SIZE_BYTES)}."
        )
        return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "show_settings":
        opts = context.user_data.get("pdf_options", {
            "page_size": DEFAULT_PAGE_SIZE, "margin_mm": DEFAULT_MARGIN_MM
        })
        kb = [
            [
                InlineKeyboardButton("PageSize: A4", callback_data="set_pagesize_A4"),
...
async def convert_to_pdf(message, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Get user options
    pdf_opts = context.user_data.get("pdf_options", {
        "page_size": DEFAULT_PAGE_SIZE, "margin_mm": DEFAULT_MARGIN_MM
    })
    page_size = pdf_opts.get("page_size", DEFAULT_PAGE_SIZE)
    margin_mm = int(pdf_opts.get("margin_mm", DEFAULT_MARGIN_MM))
    ...
    try:
        ...
        for i, file_id in enumerate(photos, start=1):
            ...
            if getattr(file, "file_size", None) and file.file_size > MAX_FILE_SIZE_BYTES:
                await message.reply_text(
                    f"Image #{i} bahut badi hai ({readable_size(file.file_size)}). "
                    f"Max allowed {readable_size(MAX_FILE_SIZE_BYTES)}."
                )
                return
            dest = Path(tmpdir) / f"img_{i}.jpg"
            ...
            if st.st_size > MAX_FILE_SIZE_BYTES:
                await message.reply_text(
                    f"Downloaded image #{i} bahut badi hai ({readable_size(st.st_size)})."
                )
                return
            if not ensure_image_dimensions(str(dest)):
                await message.reply_text(
                    f"Image #{i} ko process nahi kar paya. Try a different image."
                )
                return
            paths.append(str(dest))
        ...
        sent = await message.reply_document(
            document=InputFile(str(pdf_path)), filename="images_converted.pdf"
        )

        # Index the sent file for retrieval
        try:
            sent_file_id = None
            if sent and getattr(sent, "document", None):
                sent_file_id = sent.document.file_id
...
async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If there is a query, filter; otherwise show recent
    for slug, meta in items[:50]:
        title = meta.get("title") or slug
        if q and q not in slug and q not in title.lower():
            continue
...
def main():
    ...
    # Debug: log all updates if DEBUG_ALL is set
    if os.environ.get("DEBUG_ALL"):
        app.add_handler(MessageHandler(filters.ALL, _debug_all_updates))
    ...
    if use_webhook:
        ...
        try:
            r = requests.post(f"https://api.telegram.org/bot{token}/setWebhook",
                              data=set_hook_payload, timeout=10)
            logger.info("setWebhook response: %s", r.text)
        except Exception as e:
            logger.warning("Failed to set webhook via API: %s", e)
        ...
        app.run_webhook(listen="0.0.0.0", port=port,
                        webhook_url=webhook_url, url_path=url_path)
    else:
        logger.info("Starting in polling mode")
        app.run_polling()
